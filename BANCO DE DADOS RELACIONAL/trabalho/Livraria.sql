...
select count(idLivro) as 'Quantitativo de Livro Cadastrado' from livro;
select nome from cliente order by nome asc;
select  e.nome as editora, l.titulo as livro from editora e join livro l on e.idEditora = l.idEditora order by e.nome DESC;
select e.nome as editora, round(avg(l.preco), 2) as 'media de Preço' from editora e join livro l on e.idEditora = l.idEditora group by e.nome;
select c.nome as cliente, coalesce(sum(ip.quantidade), 0) as 'Quantidade de livros comprados' from cliente c left join pedido p on c.idCliente = p.idCliente left join itemPedido ip on p.idPedido = ip.idPedido group by c.nome;